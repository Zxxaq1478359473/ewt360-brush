...
目录里有两个 Python 文件 + 一个教程，**必须放在同一目录**使用：

| 文件 | 大小 | 行数 | 角色 |
|---|---|---|---|
| `ewt_brush_easy.py` | ~14 KB | ~340 | **遥控器**（傻瓜引导器） |
| `ewt_brush_v2.py` | ~72 KB | ~1620 | **电视机**（刷课引擎，视频/校本课时） |
| `ewt_fm_brush_tg.py` | ~6 KB | ~160 | **FM 专用**（心灵成长/生涯规划音频课时） |
| `EWT刷课使用教程（傻瓜版）.md` | — | — | 本教程 |

> 早期还有一个 `spark.py`（83KB），是更早的独立版本，v2 已把所有功能集成进自身，**不再需要它**。
>
> **什么是 FM 课时？** EWT 作业里除了视频课时，还有一小类"心灵成长/生涯规划"音频（平台叫 FM，2~6 分钟一条）。v2 引擎只处理视频（contentType 1/11），**FM（contentType 3/5）不归它管**，要用 `ewt_fm_brush_tg.py` 单独刷（详见第 12 章）。
...
### 通用流程（两端一样）
```
运行 ewt_brush_easy.py → 输入账号密码 → 自动扫描课时 → 输配置 → 回车开刷
【第 1 步】账号 / 密码        ← 输入你的账号密码（下次自动记住）
【第 2 步】识别任务           ← 自动扫描，列出所有未完成课时
...
#### 方式一：傻瓜入口自动显示（ewt_brush_easy.py）

运行后刷课期间**每 20 秒自动刷新一行进度**，长这样：
```
⏱ 03m25s  已完成 47/193  错误 0  WAF 0  运行中 4 实例
```
- `⏱ 03m25s` = 已耗时
- `已完成 47/193` = 刷完 47 个 / 总共 193 个
- `错误` = 失败课时数（>0 也不用慌，结束后自动补刷）
- `WAF` = 风控拦截次数（自动冷却重试，一般一直为 0）
- `运行中 4 实例` = 还剩几个进程在跑
- 出现 `⚠ 进度停滞？` = 连续 1 分钟没新完成，可留意网络

> 想随时暂停刷新？按 `Ctrl+C` 会退出监控（**后台实例继续刷**），按回车回到界面。
