...
| 能力 | 原始版 spark.py | V2 稳定版 | V3 正式版 ⭐ |
|---|---|---|---|
| 视频刷课 | ❌ 仅 token 顺序刷课 | ✅ 竞态爆发加速 | ✅ 同 V2 + 优化 |
| FM/板报直写 | ❌ 不处理 | ❌ 不处理 | ✅ `updateMission` 直写 100% |
| clog 播放日志补发 | ❌ 无 | ❌ 无 | ✅ 补发提高完成判定率 |
| 登录签名头 | ❌ 无 | ❌ 无 | ✅ 补齐 Web 端签名，降风控 |
| 过课检测参数 | ❌ 无 | 原始 | ✅ 校本 +2000000、type 按 contentType 计算 |
| 判定阈值 | ❌ 无 | 1.0（过严） | ✅ 0.8（平台真实阈值） |
| 进度延迟复核 | ❌ 无 | 立即重刷 | ✅ 等待+复核后再判定 |
| 进度条面板 | ❌ 无 | 单行计数 | ✅ 课时级进度条面板 |
| 配置限制 | ❌ 无 | 有上限 | ✅ 无上限，自行决定 |
| 使用复杂度 | 需手动提供 token | 需手动分片 | easy 自动多实例 + 监控 |
